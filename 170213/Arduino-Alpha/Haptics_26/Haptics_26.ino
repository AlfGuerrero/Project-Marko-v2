...
void loop()
{
  if (mySerial.available() >= 8)
  {
    ...
    if (c == 's' && isReading == false && isReading2 == false)
    {
    ...
    else if (isReading == true && isReading2 == false) {
    ...
    /*else if (c == ',') {
      ...
      vibTimer[arrayCount] = ((String)c).toInt() * 500;
      arrayCount++;
      //Serial.print(c);
    }
    mySerial.flush();
  ...
  for(int i=0;i<8;i++){
    if(vibTimer[i] != 0 && millis() - lastTime[i] < vibTimer[i]){
      Tlc.set(pinArray[i],p_fbArray[i]);
    }else if(vibTimer[i] != 0 && millis() - lastTime[i] > (vibTimer[i]*2)){
      lastTime[i] = millis();
    }else{
      Tlc.set(pinArray[i],4095);
    }
