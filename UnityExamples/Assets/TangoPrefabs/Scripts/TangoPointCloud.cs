...
public class TangoPointCloud : MonoBehaviour, ITangoPointCloud
{
    ...
    public void Start() 
    {
        ...
        m_closePoints = new Vector3[MAX_POINT_COUNT];
		...
		quadrantActive = new bool[9];
		for (int i = 0; i < 9; i++) {
			quadrantActive [i] = false;
		}
    }
    ...
    public void OnTangoPointCloudAvailable(TangoPointCloudData pointCloud)
    {
        ...
        if (m_pointsCount > 0)
        {
			...
			//GameObject.Find("Cube (7)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
            // Converting points array to world space.
            m_overallZ = 0;

			for (int f = 1; f <= 9; f++) {
				GameObject.Find ("Cube ("+f+")").GetComponent<Renderer> ().material.shader = Shader.Find ("Specular");
				GameObject.Find ("Cube ("+f+")").GetComponent<Renderer> ().material.SetColor ("_SpecColor", Color.white);
                Debug.Log("GHOST");
            }
            for (int i = 0; i < m_pointsCount; ++i)
            {
                ...
                if(point.z <= 1)
                {
                    m_closePoints[i] = unityWorldOffsetTDepthCamera.MultiplyPoint3x4(point);

					viewportPoint = Camera.main.WorldToScreenPoint(point);


					if (viewportPoint.y <= Screen.height/3)
					{
						//GUILayout.Label("Up Top");
						if (viewportPoint.x <= Screen.width / 3) {
							//GUILayout.Label("Up Top");
							quadrantActive [0] = true;
							GameObject.Find("Cube (1)").GetComponent<Renderer>().material.shader = Shader.Find("Specular");
							GameObject.Find("Cube (1)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
						} else if (viewportPoint.x <= (Screen.width/3*2))
						{
							quadrantActive [1] = true;
							GameObject.Find("Cube (2)").GetComponent<Renderer>().material.shader = Shader.Find("Specular");
							GameObject.Find("Cube (2)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
						} else if (viewportPoint.x <= Screen.width)
						{
							quadrantActive [2] = true;
							GameObject.Find("Cube (3)").GetComponent<Renderer>().material.shader = Shader.Find("Specular"); 
							GameObject.Find("Cube (3)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
						} 
					}
					else if (viewportPoint.y <= (Screen.height/3*2))
					{
                        //GUILayout.Label("Up Top");
                        if (viewportPoint.x <= Screen.width / 3)
                        {
                            //GUILayout.Label("Up Top");
                            quadrantActive[0] = true;
                            GameObject.Find("Cube (4)").GetComponent<Renderer>().material.shader = Shader.Find("Specular");
                            GameObject.Find("Cube (4)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
                        }
                        else if (viewportPoint.x <= (Screen.width / 3 * 2))
                        {
                            quadrantActive[1] = true;
                            GameObject.Find("Cube (5)").GetComponent<Renderer>().material.shader = Shader.Find("Specular");
                            GameObject.Find("Cube (5)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
                        }
                        else if (viewportPoint.x <= Screen.width)
                        {
                            quadrantActive[2] = true;
                            GameObject.Find("Cube (6)").GetComponent<Renderer>().material.shader = Shader.Find("Specular");
                            GameObject.Find("Cube (6)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
                        }
                    }
					else if (viewportPoint.y <= Screen.height)
					{
                        //GUILayout.Label("Up Top");
                        if (viewportPoint.x <= Screen.width / 3)
                        {
                            //GUILayout.Label("Up Top");
                            quadrantActive[0] = true;
                            GameObject.Find("Cube (7)").GetComponent<Renderer>().material.shader = Shader.Find("Specular");
                            GameObject.Find("Cube (7)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
                        }
                        else if (viewportPoint.x <= (Screen.width / 3 * 2))
                        {
                            quadrantActive[1] = true;
                            GameObject.Find("Cube (8)").GetComponent<Renderer>().material.shader = Shader.Find("Specular");
                            GameObject.Find("Cube (8)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
                        }
                        else if (viewportPoint.x <= Screen.width)
                        {
                            quadrantActive[2] = true;
                            GameObject.Find("Cube (9)").GetComponent<Renderer>().material.shader = Shader.Find("Specular");
                            GameObject.Find("Cube (9)").GetComponent<Renderer>().material.SetColor("_SpecColor", Color.red);
                        }
                    }
                ...
                else if (point.z < m_closestZ.z && Mathf.Abs(point.z - m_closestZ.z) < 1.0f)
                {
                    m_closestZ = point;
                }
    ...
    void OnGUI()
    {
        ...
        GUI.color = Color.red;
        GUILayout.Button(m_pointsCount.ToString());
        GUILayout.Label(m_closestZ.ToString());
		//for (int i = 0; i < 9; i++) {
			//if (quadrantActive [i] == true) {
				//GUILayout.Label (i.ToString ());
			//}
		//}
		GUILayout.Label("THIS" + viewportPoint.ToString());
        GUILayout.Label("THAT" + Screen.width.ToString());
        //GUILayout.Label("AND SUCH" + Screen.width.ToString());
        GUILayout.Label(m_closestZ.z.ToString());
